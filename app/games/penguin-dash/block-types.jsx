...
function ThreeOptionsLayout({ children }) {
  return (
    <div className="grid grid-cols-3 grid-rows-2 h-[82px]">
      <div className="row-span-2 flex justify-center">{children[0]}</div>
      <div className="row-span-2 flex justify-center">{children[1]}</div>
      <div className="row-span-2 flex justify-center">{children[2]}</div>
    </div>
  );
}
...
const OptionButton = (props) => {
  const { isActive, option, handleOptionButtonClick, row, col, optionsLength } =
    props;
  ...
  return (
    <div
      ...
      onClick={
        isActive ? () => handleOptionButtonClick(row, col, optionsLength) : null
      }
    >
      <img src={`/images/penguin-dash/ice-type-${iceType.current}.gif`} />
      <p
        className="absolute top-2 text-xl font-bold"
...
const FinishPlace = ({ handleClick, isFinish }) => {
  return (
    ...
    >
      {isFinish ? (
        <motion.p
          animate={{ y: [0, -20, 0] }} // move up (-10px) then back to 0
          transition={{
            duration: 2, // full cycle duration
            repeat: Infinity, // loop forever
            ease: "easeInOut", // smooth easing
          }}
          className="absolute -top-16 z-5 text-lg"
        >
          Click the flag to finish
        </motion.p>
      ) : null}
      ...
      <img
        ...
        alt="Finish flag"
        className="absolute -top-12 left-1/2 w-[72px] h-[100px]"
      />
    </div>
  );
};
