...
export default function GameGrid({ level, phase, paths, setPaths }) {
  const boxSelectionSound = useRef(
    new Howl({ src: "/sounds/cancel-selection-pop.mp3" })
  );
  ...
  const handleEnter = (row, col) => {
    ...
    if (alreadyInPath) {
      const isEndOfSamePair = cellSymbol === activePath.id && !isStartCell;
      ...
      {grid.flat().map((cell, i) => {
        ...
        return (
          <motion.div
            ...
            onTouchStart={() => handleStart(cell.row, cell.col)}
            onTouchMove={(e) => {
              ...
              if (row !== null && col !== null) {
                handleEnter(Number(row), Number(col));
              }
            }}
          >
            {pair && (
