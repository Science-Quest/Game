...
export default function BoxRecallApp(params) {
  ...
  const isCorrect =
    getNumOfCorrectAnswer(selectedBoxes, levelData.targets) ===
    levelData.targets.length;
...
function getNumOfCorrectAnswer(answer, target) {
  return answer.filter((a) =>
    target.some((t) => t.row === a.row && t.col === a.col)
  ).length;
}

function calculatePenaltiedScore(playTimeInSeconds, selectedBoxes, levelData) {
  const correctAnswers = getNumOfCorrectAnswer(
    selectedBoxes,
    levelData.targets
  );
